...
let main _ =
    ...
    let arrP = scanner.ArrayF64(n)
    let cums = Array.scan (+) 0.0 arrP
    let f d = (d + 1.0) / 2.0
    let p = n - k + 1
    let ad = [|for i in 0..p-1 -> cums.[k + i] - cums.[i]|]
    let mx = Array.max ad
    let adind = Array.findIndex (fun x -> x = mx) ad
    let narr = arrP.[adind..(adind + k - 1)]
