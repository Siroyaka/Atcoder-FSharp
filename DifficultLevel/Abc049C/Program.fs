...
let main _ =
    ...
    let scanner = Scanner()
    let s = scanner.Next().ToCharArray() |> Array.rev |> Array.toList
    let rec find =
        function
        | [] -> true
        | 'm':: 'a':: 'e':: 'r':: 'd' :: tail -> find tail
        | 'r':: 'e':: 'm':: 'a':: 'e':: 'r':: 'd' :: tail -> find tail
        | 'e':: 's':: 'a':: 'r':: 'e' :: tail -> find tail
        | 'r':: 'e':: 's':: 'a':: 'r':: 'e' :: tail -> find tail
        | _ -> false

    if find s then "YES" else "NO"
    |> printfn "%s"
    0
