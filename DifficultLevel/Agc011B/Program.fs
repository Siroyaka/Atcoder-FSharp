...
let main _ =
    ...
    let arr = [for _ in 0..n-1 -> scanner.NextI64()] |> List.sort |> List.toArray
    let cums = Array.scan (+) 0L arr |> Array.zip [|0..n|]
    let arradd = Array.append arr [|0L|]

    cums
    |> Array.fold (fun acum (i, x) -> if x * 2L < arradd.[i] then i else acum) 0
    |> fun x -> n - x
    |> printfn "%i"
