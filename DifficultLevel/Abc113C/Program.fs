...
let main _ =
    ...
    let arriPY = [|for i in 0..m-1 -> i, scanner.NextI32(), scanner.NextI64()|]
    let f a = (string a).PadLeft(6, '0')

    arriPY
    |> Array.groupBy (fun (_, i, _) -> i)
    |> Array.collect (fun (_, b) ->
        b
        |> Array.sortBy (fun (_, _, x) -> x)
        |> Array.mapi (fun i (x, y, _) -> x, (f y) + (f (i + 1)))
    )
    |> Array.sortBy fst
    |> Array.map snd
    |> String.concat "\n"
