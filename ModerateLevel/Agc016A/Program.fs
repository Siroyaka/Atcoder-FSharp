...
let main _ =
    ...
    let rec func c mxcount count =
        function
        | [] -> max mxcount count
        | x::xs when x = c -> func c (max mxcount count) 0 xs
        | _::xs -> func c mxcount (count+1) xs

    let arr = scanner.Next() |> Seq.toList
    let f = fun x -> func x 0 0 arr
