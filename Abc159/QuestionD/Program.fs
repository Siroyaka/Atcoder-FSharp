...
let main _ =
    ...
    let arrA = scanner.ArrayI32(n)
    let bx: int64 array = Array.zeroCreate n
    for i in arrA do bx.[i-1] <- bx.[i-1] + 1L
    let sums = Array.fold (fun acc x -> acc + f x) 0L bx

    for i in 0..n-1 do
        let a = bx.[arrA.[i] - 1] - 1L
        sums - a
        |> printfn "%i"
